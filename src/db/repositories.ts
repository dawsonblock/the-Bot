...
import type { EventRead } from "../models/event.model.js";
...
export class TraceRepository {
  ...
  async getTrace(traceId: string): Promise<TraceRead | null> {
    return db
      ...
      .then(([trace]) =>
        trace ? { ...trace, metrics: asRecord(trace.metrics) } : null,
      );
  }
  ...
  async listTraces(limit = 50): Promise<TraceRead[]> {
    return db
      ...
      .limit(limit)
      .then((rows) =>
        rows.map((trace) => ({ ...trace, metrics: asRecord(trace.metrics) })),
      );
  }
...
export class EventRepository {
  async getEventsForTrace(traceId: string): Promise<EventRead[]> {
    return db
      ...
      .then((rows) =>
        rows.map((event) =>
          eventFromSelect(event as unknown as Record<string, unknown>),
        ),
      );
  }
